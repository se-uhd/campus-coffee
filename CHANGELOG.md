...
- Begin migrating the production code from Java to Kotlin, module by module, using a "models last" strategy: convert everything that is not a record-based model first, and keep the record types and the tests in Java until a final pass. So far converted: the `domain` module's enums, exceptions, ports, and service implementations; the `api` module's controllers (`CrudController` and the concrete controllers) and `Dto`/`DtoMapper` interfaces; the `data` module's repositories, data-service implementations (including the generic `CrudDataServiceImpl`), OpenStreetMap HTTP client, and repository configuration; and the `application` module's `Application` entry point and `LoadInitialData`. Still Java for now: the record types (the `domain` models, the `api` DTOs, `ErrorResponse`), the JPA entities and their sequence generators, the MapStruct mappers (DTO and entity) with `HouseNumberConverter`, `TestFixtures`, the `GlobalExceptionHandler`, and the OpenAPI customizers; Kotlin reads their Lombok-generated builders and record accessors during the mixed-source window. Add a `kotlin-conventions` convention plugin (Kotlin 2.3.20, `jvmTarget` 25, the Spring all-open and Lombok compiler plugins, `kotlin-reflect`).
